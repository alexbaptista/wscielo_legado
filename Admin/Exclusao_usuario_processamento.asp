...
SQL = "SELECT * FROM Dados WHERE Usuario = '" & Request.Form("oldusuario") & "'"
rs.Open SQL, conn
...
Else
	...
	SQL = "DELETE * FROM Dados WHERE Usuario = '" & Request.Form("oldusuario") & "'"
	conn.Execute(SQL)
	conn.close
	...
	<script language="javascript">
	<!--
	alert(" Login: (\"<%= Request.Form("oldusuario") %>\") foi excluso com sucesso !"); 
	window.location = "Opcoes_usuario.asp";
	-->
	</script>
	</body>
